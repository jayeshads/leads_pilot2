    ...
    for model in models_to_try:
        ...
        try:
            ...
            if resp.status_code == 429:
                last_error_detail = f"Rate limited (429) on {model}"
                continue
            ...
            if e.response is not None and e.response.status_code == 429:
                last_error_detail = f"Rate limited on {model}: {detail}"
                continue
            raise LLMError(f"OpenRouter API request failed on {model}: {e} {detail}".strip()) from e
            
    raise LLMError(f"OpenRouter API failed on all fallback models due to rate limits. Last error: {last_error_detail}")
